...
function initView(b, c) {
    ...
    if (k) {
        Event.observe(k, "click", function(n) {
            Event.stop(n);
            openGroupActionDialog("/groups/actions/confirm_leave", "/groups/actions/leave", null, b, null)
        })
    }
    ...
    if (f) {
        Event.observe(f, "click", function(n) {
            Event.stop(n);
            openGroupJoinDialog("/groups/actions/join", b)
        })
    }
...
var MembersList = {
    ...
    confirm: function(b) {
        ...
        if (d.length > 0) {
            ...
            new Ajax.Request(habboReqPath + "/myhabbo/groups/batch/confirm_" + b, {
                method: "post",
                parameters: {
                    groupId: MembersList.groupId,
                    targetIds: d
                ...
                onComplete: function(f, e) {
                    ...
                    if ($("group-action-ok")) {
                        Event.observe($("group-action-ok"), "click", function(g) {
                            Event.stop(g);
                            Dialog.setAsWaitDialog(c);
                            new Ajax.Request(habboReqPath + "/myhabbo/groups/batch/" + b, {
                                method: "post",
                                parameters: {
                                    groupId: MembersList.groupId,
                                    targetIds: d
...
var Discussions = {
    ...
    save: function(a) {
        ...
        if (a != null) {
            g = "topic";
            c += "&topicName=" + a;
            d = habboReqPath + "/discussions/actions/savetopic"
        } else {
...
var MemberWidget = Class.create(FriendsWidget, {
    options: {
        ...
        ownerParameter: "&_groupspage_requested_group="
    },
            ...
            onSuccess: function(d) {
                ...
                if (c == "valid" && $("profile-tags-status-field")) {
                    new Ajax.Updater("profile-tag-list", "/myhabbo/tag/list", {
                        parameters: "tagMsgCode=" + encodeURIComponent("valid") + "&accountId=" + b
                    })
                }
                this.errorMessage(c)
            }.bind(this)
