<?php
$PageName = 'Catalogo <sub> Main Sections </sub>';
...
<table>
		...
		<tbody>
			<?php
			$GrandParentQuery = SQL::query("SELECT * FROM catalog_pages WHERE parent_id = -1 ORDER BY order_num ASC");

			if($GrandParentQuery->num_rows > 0):
				while ($row = $GrandParentQuery->fetch_assoc()): ?>
					<tr>
						...
						<td class="right"><?php if(User::hasPermission('ase.edit_delete')): ?><a href="<?php echo HHURL; ?>/manage?p=catalog_m&do=edit&key=<?php echo $row['id']; ?>">Editar</a> || <a href="#" onclick="NewDialog('Aviso', 'Estás seguro de querer cerrar el emulador ¡No les gustará a tus usuarios!', '', '<a class=\'button alert\'href=\'<?php echo HHURL; ?>/manage?p=catalog_m&do=remove&key=<?php echo $row['id']; ?>\'>Borrar</a><button class=\'button alert\' onclick=\'CloseDialog()\'>Cerrar</button>');">Borrar</a><?php endif; ?></td>
					</tr>
			<?php endwhile; endif; ?>
		</tbody>
</table>
...
<ul class="concertina" id="accordionid" data-role="accordion">
	<li class="concertinali">
		...
		<div class="concertinaccn">
			<form action='<?php echo HHURL; ?>/manage?p=catalog_m&filter=<?php echo $newRow['parent_id']; ?>&do=edit&key=<?php echo $key; ?>' method='post' name='theAdminForm' id='theAdminForm'>
				...
				<table>
					<tbody>
						...
						<tr>
							<td>
								<b>Sub Páginas</b>
								<div class="graytext">Que páginas estan dentro de esta página principal.</div>
							</td>
							<td>
								<a href="<?php echo HHURL; ?>/manage?p=catalog_sm&filter=<?php echo $key; ?>">Ver Sub Páginas...</a>
							</td>
						</tr>
						<tr>
