<?php
$PageName = 'Catalogo <sub> Sub Main Sections </sub>';
	...
	if($filterQuery && $filterQuery->num_rows > 0):
		while ($Row = $filterQuery->fetch_assoc()):
			$DataHTML .= '<tr class="hover">
								...
								<td class="right"><a href="' . HHURL . '/manage?p=catalog_sm&filter=' . $parentid . '&do=edit&key=' . $Row['id']. '">Editar</a></td>
							</tr>';
		endwhile;
	else:
		...
		<tbody>
			...
			if($GrandParentQuery->num_rows > 0):
				while ($GrandParentRows = $GrandParentQuery->fetch_assoc()):
					$ParentQuery = SQL::query('SELECT * FROM catalog_pages WHERE parent_id = \'' . $GrandParentRows['id'] . '\' ORDER BY order_num ASC');
					if($ParentQuery->num_rows > 0):
						while($row = $ParentQuery->fetch_assoc()):
			?>
							<tr class="hover">
								...
								<td class="right"><?php if(User::hasPermission('ase.edit_delete')): ?><a href="<?php echo HHURL; ?>/manage?p=catalog_sm&do=edit&key=<?php echo $row['id']; ?>">Editar</a><?php endif; ?></td>
							</tr>
			<?php endwhile; endif; endwhile; endif; else: echo $DataHTML; endif;?>
		</tbody>
</table>
...
<ul class="concertina" id="accordionid" data-role="accordion">
	<li class="concertinali">
		...
		<div class="concertinaccn">
			<form action='<?php echo HHURL; ?>/manage?p=catalog_sm&filter=<?php echo $newRow['parent_id']; ?>&do=edit&key=<?php echo $key; ?>' method='post' name='theAdminForm' id='theAdminForm'>
				...
				<table>
					<tbody>
						...
						<tr>
							...
							<td>
								<a href="<?php echo HHURL; ?>/manage?p=catalog_ssm&filter=<?php echo $key; ?>">Ver Sub Páginas...</a>
							</td>
						</tr>
						<tr>
